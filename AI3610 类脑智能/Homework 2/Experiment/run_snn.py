...
from model.snn import MNIST_SNN
...
def run_snn(args):
    ...
    save_path = os.path.join(args.save_path, 'snn_{}'.format(time_stamp))
    ...
    # load dataset
    dataset_train = NMNIST('./dataset', train=True, data_type='frame', frames_number=args.time_steps, split_by='number')
    dataset_test = NMNIST('./dataset', train=False, data_type='frame', frames_number=args.time_steps, split_by='number')
    ...
    for epoch in range(args.num_epochs):
        ...
        for images, labels in loader_train:
            ...
            optimizer.zero_grad()
            outputs = model(images).mean(dim=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--time_steps', default=16, type=int, help='simulation time steps')
    parser.add_argument('-n', '--spiking_neuron', default='LIF', type=str, choices=['IF', 'LIF'], help='spiking neuron')
